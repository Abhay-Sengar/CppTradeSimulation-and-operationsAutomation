...
// system noise. Run on an isolated core (taskset -c 10) for the steadiest read.
//
...
double bench(const TscClock& clk, Body body) {
    // warm-up
    for (u64 i = 0; i < 50'000; ++i) body(i);
    u64 best = ~u64{0};
    for (int t = 0; t < kTrials; ++t) {
        const u64 s = rdtsc_serialized();
        for (u64 i = 0; i < kIters; ++i) body(i);
...
int main() {
    ...
    const double ns_crtp = bench(clk, [&](u64 i) {
        ...
        g_sink = g_sink + static_cast<u64>(o.px);
    });
    ...
    const double ns_umap = bench(clk, [&](u64 i) {
        ...
        if (it != umap.end()) g_sink = g_sink + it->second;
    });
